...
class DeepSort(object):
    ...
    def update_tracks(self, raw_detections, embeds=None, frame=None, today=None, others=None, instance_masks=None, register_person=False):
        ...
        matches, reg_info = self.tracker.update(detections, today=today, frame=frame, register_person=register_person)

        detection_ids = []  # [(detection index, track ID)]
        for idx, (track_idx, detection_idx) in enumerate(matches):
            detection_ids.append(indices[detection_idx], int(self.tracker.tracks[track_idx].track_id))

        return self.tracker.tracks, detection_ids, reg_info
        ...
        for i, (raw_det, embed) in enumerate(zip(raw_dets, embeds)):
            detection_list.append(
                Detection(  
                    ...
                    raw_det[1], 
                    embed, 
                    class_name=raw_det[2] if len(raw_det)==4 else None,
